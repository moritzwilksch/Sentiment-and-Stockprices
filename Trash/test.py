...
if False:
    ...
    with open('data/tokenized_tweets.pickle', 'wb') as f:
        pickle.dump(tokenized_tweets, f)

with open('data/tokenized_tweets.pickle', "rb") as f:
    tokenized_tweets = pickle.load(f)
...
performance = yf.Ticker('TSLA').history(start=df.date.min(), end=df.date.max())
performance = performance.Close
performance = performance.shift(-1)/performance
performance = (performance > 1).map({True: 1, False: 0})
...
embedding.fit(xemb_train, ytrain, validation_data=(xemb_test, ytest), epochs=20, batch_size=128)
