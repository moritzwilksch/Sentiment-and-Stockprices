...
# %%
daily_sentiment = vader_df.groupby(vader_df.datetime.dt.date).agg(
...
df = pd.DataFrame({
    ...
    'prev1': raw_prices.pct_change().shift(0).asfreq('D', method='ffill').reset_index(drop=True).fillna(0),
    'prev2': raw_prices.pct_change().shift(1).asfreq('D', method='ffill').reset_index(drop=True).fillna(0),
    'prev3': raw_prices.pct_change().shift(2).asfreq('D', method='ffill').reset_index(drop=True).fillna(0),
...
for combo in combos:
    ...
    # LR
    lr = LinearRegression()
    results['lr_score'].append(-np.mean(cross_val_score(lr, X, y, n_jobs=-1, cv=cv, scoring='neg_mean_absolute_error')))

    # NB
    nb2 = GaussianNB()
    results['nb_score'].append(
        np.mean(cross_val_score(nb2, X, y_binary, cv=cv, n_jobs=-1, scoring=SCORING))
        )
